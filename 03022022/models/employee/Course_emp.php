...
class Course_emp extends CI_Model
{
	...
	public function get_courses_free()
 	...
 	public function get_courses_paid_all($id)
 	{
  	...
  	public function get_course_assign($id,$comp_id)
  	{
  	...
  	// {
  	// 	$result=$this->db->query("select emp.course_id,emp.emp_id,video.*,status.status as video_status  from  course_assign_video_emp as emp  left join course_video_courses as video   on emp.course_id=video.id  left join course_emp_video_status as status on status.emp_id='".$id."' and status.course_id=emp.course_id where emp.emp_id='".$id."' or emp.emp_id=0 group by emp.course_id ");
 		// return $result->result_array();
  	// }
