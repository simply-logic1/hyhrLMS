...
class Client_model extends CI_Model
{
  ...
  public function get_list_vid($id)
  {
   ...
   $result=$this->db->select('COUNT(id) as list')->from('course_video_courses')->get();
	...
	{
		$result=$this->db->query("select * from course_video_courses where id='".$id."'");
   ...
   public function check_invite_email($compid,$mail)
   {
	    ...
	    $query=$this->db->select('invite_status,status')->from('course_employee')->where($check);
	    $get= $query->get();
	    $count=$get->num_rows();
   ...
   public function check_all_user_mail($email)
   {
   	    ...
   	    $query=$this->db->select('*')->from('course_user_acc')->where('primary_email',$email)->or_where('company_email',$email)->get();
   ...
   public function validate_user_email($email)
   {
   	...
   	$query=$this->db->select('status')->from('course_clients')->where('email_id',$email)->or_where('email_id',$email);
	    $get= $query->get();
		$count=$get->num_rows();
  ...
  public function get_id_forgot($email)
  {
    ...
    public function update($tblname,$data,$id)
   {
	...
	public function get_test_report($emp_id)
	{
		...
		->join('course_video_courses as video','video.id=test.video_id')
