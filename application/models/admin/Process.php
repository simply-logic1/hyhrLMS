...
class Process extends CI_Model
{
   ...
   public function course_get_content_cat()
   {
	...
	public function count_list($fld,$val,$tblname)
	{
		$result=$this->db->select('COUNT(id) as no_of_list')->where( $fld,$val)->get($tblname);
	...
	public function get_test_report($emp_id)
	{
		...
		$query=$this->db->select("res.id as rid,emp.id as empid,video.*,video_list.*,video_status.*,test.id as tid,
 ...
 public function delete_client_data($tblname,$ids)
{
	...
	{ 
		...
		$check="is_reminder= 0 and view_status=0 and assign.createdon >= DATE_SUB(NOW(),INTERVAL 24 HOUR)";
		$res1=$this->db->select('TIMESTAMPDIFF(hour, assign.createdon, now()) as timediff,emp.first_name,emp.last_name,assign.createdon,emp.primary_mail,emp.company_mail,post.*')
		->join('course_employee as emp','emp.id=assign.emp_id','left')
		->join('course_content as post','post.id=assign.course_id','left')
					->from('course_assign_video_emp as assign')
					->where($check )
					   ->get();
