...
class Course_emp extends CI_Model
{
	...
	public function get_courses_free()
	  ...
	  */
	   ...
	   public function get_document_assign($id)
  	 {
  		$result=$this->db->query("select emp.createdon as assign_date,emp.course_id,emp.emp_id,video.*,status.status as video_status  from  course_assign_video_emp as emp  left join course_content as video   on emp.course_id=video.id  left join course_emp_video_status as status on status.emp_id='".$id."' and status.course_id=emp.course_id where video.course_type='document' and emp.emp_id='".$id."' or emp.emp_id=0 group by emp.course_id ");
 		return $result->result_array();
  	 }
	  ...
	  public function get_test_status($uid,$id)
  {
	...
	public function get_data_all_video($id)
	{
		$result=$this->db->query("select emp.createdon as assign_date,emp.course_id,emp.emp_id,video.*   from  course_assign_video_emp as emp  left join course_content as video   on emp.course_id=video.id  where video.course_type='video' and emp.emp_id='".$id."' or emp.emp_id=0 group by emp.course_id ");
		return $result->result_array();
	} 
	
	public function get_data_user_assign($tblname)
	{
		$query=$this->db->get($tblname);
 		$userdata=$query->result_array();
		return $userdata;
	...
	public function get_data_field_useid($fld,$val,$tblname,$id){
	 
		$result=$this->db->query("select emp.course_id,emp.emp_id,video.*  from  course_assign_video_emp as emp  left join course_media as video   on emp.course_id=video.id  where emp.course_type='document' and emp.emp_id='".$id."' or emp.emp_id=0 group by emp.course_id ");
		return $result->result_array();
	}
	public function get_category_list($id,$cid)
	{
		$result=$this->db->query("select cats.category_name,emp.createdon as assign_date,emp.course_id,emp.emp_id,video.*   from  course_assign_video_emp as emp  left join course_content as video   on emp.course_id=video.id    left join course_cat_links as cat   on video.id=cat.post_id and   cat.cat_id='".$cid."' left join course_category as cats   on cats.id=cat.cat_id and cats.id='".$cid."'  where   cat.cat_id='".$cid."' and emp.emp_id='".$id."' or emp.emp_id=0 group by emp.course_id ");
		return $result->result_array();
	}
	public function get_category_list_video($id,$cid)
	{
		$result=$this->db->query("select cats.category_name,emp.createdon as assign_date,emp.course_id,emp.emp_id,video.*   from  course_assign_video_emp as emp  left join course_media as video   on emp.course_id=video.id  and video.media_type='document' left join course_cat_links as cat   on video.id=cat.post_id and   cat.cat_id='".$cid."' left join course_category as cats   on cats.id=cat.cat_id and cats.id='".$cid."'  where  emp.course_type='document' and  cat.cat_id='".$cid."' and emp.emp_id='".$id."' or emp.emp_id=0 group by emp.course_id   ");
		return $result->result_array();
	}
}
