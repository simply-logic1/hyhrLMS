   ...
   redirect('admin/videos');
    ...
    public function add_content()
    {
      ...
      $video=$this->input->post('course_video');
            ...
            /* $image = UploadedFile::getInstanceByName('thumb_img');
...
/*
      ...
      if($_FILES['thumb_img']['name'] !='')
          ...
          else
          {
                  ...
                  $fileName = $_FILES['thumb_img']['name'];
                  $ext = pathinfo($fileName, PATHINFO_EXTENSION);
                  $tmpName = $_FILES['thumb_img']['tmp_name'];
                  $fileType = $_FILES['thumb_img']['type'];
                  $fileName = addslashes($fileName);
               
                  $public_id = "myT".time();
           
                  $thumb_name=$public_id;
       ...
       public function add_doc_data()
       {
        ...
        {
          $config['upload_path'] = 'asset/course_content/'; 
                                  $config['allowed_types'] = 'mp4|m4v|mov|wmv|avi|mpg|ogv|3gp|3g2|jpg|png|gif|zip|pdf|doc|docx|csv|xml';
                                 // $config['encrypt_name'] = TRUE; 
                                 $config['max_size']  = 1024002;
                                 $config['max_width']  = 6000;
                                 $config['max_height'] = 6000;
            ...
            else
            {
                ...
                $file_name=  $filename2;
                  ...
                  $media_title=$this->input->post('name');
                   $data=array(
                      'course_title'=>$media_title,
                      'course_type'=>"document",
                      'category'=>$category,
                        
                        'course_filename'=>$file_name,
                        'created_on'=>$now
