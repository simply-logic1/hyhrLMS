...
*/
   ...
   $result=$this->db->query("select * from course_video_courses"); 
    ...
    public function get_survey_details( $video_id)

    {

      $check=array('test.id'=>$video_id);
    ...
    $result=$this->db->select('test.*,test.id as tid,questions.id as qid,questions.*,COUNT(*) as no_of_ques,answers.*,
      ...
      GROUP_CONCAT(CONCAT(answers.id,":",answers.option) ORDER BY answers.id)  as allanswer')

         ->join('course_survey_question as questions','questions.test_id=test.id','left')

          ->join('course_survey_option as answers','answers.question_id=questions.id','left')
 

          ->group_by('questions.id')

        ->where($check)->get('course_survey as test');
    ...
    public function delete_test_data($ids,$tblname)
 ...
 public function delete_ques_data($ids)
  ...
  public function check_surveyid_exist($cid,$vid)
  ...
  public function get_survey_questions($client_id,$video_id,$qid)

  {

    $check=array( 'test.id'=>$video_id,'test.status'=>1);
